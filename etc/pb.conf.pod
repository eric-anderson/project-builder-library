...
Key: OS (could be from the most generic up to the most specific from ostype, osfamily, os, os-ver, os-ver-arch). The family name is generaly used here.
 Value: comma separated list of URLs that point to repository files, or packages to install. The values may not include subsitutions.
 Conffile: project
 Example: addrepo centos-5-x86_64 = http://localhost/extras.repo,http://mirror.centos.org/centos/5.8/extras/x86_64/RPMS/chrpath-0.13-3.el5.centos.x86_64.rpm
...
=item B<install_deps_hook>

 Nature: Optional
 Key: OS (could be from the most generic up to the most specific from ostype, osfamily, os, os-ver, os-ver-arch). The family name is generaly used here.
 Value: string indicating the command to execute before trying to install package dependencies.
 Conffile: TODO-reviewer: ?
 Example: install_deps_hook ubuntu = sudo dpkg-divert --local --rename --add /sbin/initctl; sudo ln -sf /bin/true /sbin/initctl
...
=item B<pbdebcomponent>

 Nature: Optional
 Key: project (as defined in the -p option or PBPROJ environment variable)
 Value: The component in the debian/ubuntu repository, e.g. main for free software, non-free for non-free software, or contrib for free software that depends on non-free software; default = contrib; TODO-reviewer: shouldn't the default be main assuming open-source?
 Conffile: project
 Example: pbdebcomponent Lintel = main
...
=item B<pbgpgcheck>
 ...
 Nature: Optional
 Key: project (as defined in the -p option or PBPROJ environment variable)
 Value: Whether the repository file should be generated specifying that gpg checking of the packages is on. Note that failures to sign packages is tolerated by default, which means that signatures can fail making the repository file generated not work.  The default is to require gpg checks.
 Conffile: project
 Example: pbdebcomponent Lintel = 0
...
=item B<pbpasspath>
 ...
 Nature: Optional
 Key: project (as defined in the -p option or PBPROJ environment variable)
 Value: The directory under which will be found your secret key file.
 Conffile: home
...
=item B<sshlogin>
 ...
 Nature: Optional
 Key: project (as defined in the -p option or PBPROJ environment variable)
 Value: login to use when connecting to the repository server B<sshhost> for package delivery; default `whoami`
 Conffile: project
...
=item B<sshport>
 ...
 Nature: Optional
 Key: project (as defined in the -p option or PBPROJ environment variable)
 Value: port to use when connecting to the repository server B<sshhost> for package delivery; default 22
 Conffile: project
...
=item B<vedebtype>
 ...
 Nature: Optional
 Key: project (as defined in the -p option or PBPROJ environment variable)
 Value: debootstrap (no other tool to create deb distro based chroot, so this is the default)
 Conffile: ve
...
# TODO-reviewer: who else should have put copyrights in here?
